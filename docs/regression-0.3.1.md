# Review 0.3.1 regression report

Added `e` to reveal all context in the file at the top of the review viewport.
Code ranges and saved comments remain unchanged; other files retain their
existing visibility. Input fields continue to accept `e` as text.

Independent review found two viewport issues, both fixed before final testing:

- A trailing expansion row could anchor to the next file; it now anchors to a
  revealed row in the same file.
- Centering after mouse scrolling could leave a stale sidebar file index; the
  expansion target now comes directly from the viewport.

Re-review found no further actionable issues. New unit tests cover selection,
comments, viewport preservation, unrelated files, input modes, empty files,
repeated expansion, disappearing expansion rows, and stale sidebar state.
...
| `nix develop -c cargo test --all-targets` | PASS: 80 unit, 6 CLI integration, 15 Git integration tests |
| `nix develop -c cargo build --release` | PASS |
| `target/release/review --help` | PASS |
| `target/release/review --version` | PASS: review 0.3.1 |
| `nix eval --raw .#packages.x86_64-linux.review.version` | PASS: 0.3.1 |
| `git diff --check` | PASS |
