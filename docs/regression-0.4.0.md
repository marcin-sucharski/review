# Review 0.4.0 regression report

Added stacked PR reviews with source-branch selection followed by target-branch
selection. Both pickers retain existing filtering and ordering. Comparisons use
frozen source/target commits and the merge base, exclude working-tree changes,
and never check out another branch. Markdown/XML identify both selected branches;
stacked output also includes exact commit IDs and the merge base. Archive history
uses the reviewed source branch. Existing current-branch reviews now identify
both source and target as well.

Independent review found two edge cases, fixed and re-reviewed:

- Resolve Git-qualified branch names produced when branches share names with tags.
- Render Markdown source labels as safe code spans so legal markup characters in
  branch names remain literal.

New tests cover stacked layers, diverged targets, dirty unrelated checkout,
remote/qualified branches, invalid refs and empty comparisons, picker ordering,
sequential CLI answers, Markdown/XML identities, and archive source metadata.
Version is 0.4.0 in Cargo.toml, Cargo.lock, and flake.nix.
...
| `nix develop -c cargo test --all-targets` | PASS: 84 unit, 7 CLI integration, 18 Git integration tests |
| `nix develop -c cargo build --release` | PASS |
| `target/release/review --help` | PASS |
| `target/release/review --version` | PASS: review 0.4.0 |
| `nix eval --raw .#packages.x86_64-linux.review.version` | PASS: 0.4.0 |
| `git diff --check` | PASS |
